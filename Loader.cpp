...
Scrooge* Loader::load(std::string levelName)
{
    if(levelName=="theMoon")
    {
        ...
        //TEMPORARY BLOCKS-->SENZA QUESTI GLI ALIENI NON HANNO IL RIFERIMENTO SU COSA SBATTERE PER CAMBIARE DIREZIONE
        new Block(QPointF(55*TILE, 53*TILE), 0*TILE, 0.5*TILE);
        new Block(QPointF(25.0 *TILE, 51*TILE), 0*TILE, 0.5*TILE);
        new Block(QPointF(30.0 *TILE, 51*TILE), 0*TILE, 0.5*TILE);
        new Block(QPointF(30.0 *TILE, 51*TILE), 0*TILE, 0.5*TILE);
